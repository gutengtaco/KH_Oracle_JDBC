...
public class MemberController {
	...
	public void insertMember(String userId, String userPwd, String userName,
		...
		// 2) Service의 메소드 호출(이때, 위에서 가공된 Member객체를 매개변수로 넘김)
		int result = new MemberService().insertMember(m);
		
		// 3) Service로부터 반환받은 리턴값이 성공이면 성공, 실패면 실패라는 응답화면 지정
		//    => View의 메소드 호출
		if(result > 0) { // 성공
	...
	public void selectAll() {
		...
		// 3) Service로부터 반환받은 리턴값이 있는지, 없는지를 응답화면으로 지정
	...
	public void selectByUserId(String userId) {
		...
		*/
		...
		// 3) Service에서 반환한 결과값이 있는지 없는지 판단 후 View에 메소드 호출
		if(m != null ){
	...
	public void selectByUserName(String keyword) {
		...
		*/
		...
		// 3) Service에서 반환된 결과값이 있는지 없는지 판단 후 View에 메소드 호출
		if(list.isEmpty()) {
	...
	public void updateMember(String userId, String newPwd, String newEmail,
		...
		// 2) Service에 메소드를 호출(가공한 vo를 매개변수로 넘길 예정)
